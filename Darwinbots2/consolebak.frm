...
Private Sub evnt_textentered(ind As Integer, text As String)
  ...
  Select Case text
    ...
    Case "pause"
      Form1.Active = False
    Case "set"
      rob(ind).mem(variabile(ind, rob(ind).console.text(2))) = val(rob(ind).console.text(3))
...
Public Sub cycle(num As Integer)
  ...
  For k = 1 To num
      ...
      Form1.DrawShots
      Form1.DrawAllRobs
      feedvegs TotalEnergy, totvegs
      If datirob.Visible Then
        datirob.infoupdate rob(q).AbsNum, rob(q).nrg, rob(q).Parent, rob(q).Mutations, rob(q).age, rob(q).SonNumber, 1, rob(q).fname, rob(q).genenum, rob(q).LastMut, rob(q).generation, rob(q).DnaLen, rob(q).LastOwner, rob(q).Waste
      End If
