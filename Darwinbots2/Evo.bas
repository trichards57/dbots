...
'Botsareus 9/6/2014 New idea, make sure all stronger IM base robots get to update the system
If y_eco_im > 0 Then
...
'Step2 calculate cumelative genetic distance
  Form1.GraphLab.Visible = True
    ...
    For t = 1 To MaxRobs
     If rob(t).exist And Not rob(t).Veg And Not rob(t).FName = "Corpse" And rob(t).FName = "Base.txt" Then
              'calculate cumelative genetic distance
              For t2 = 1 To MaxRobs
               If t <> t2 Then
                If rob(t2).exist And Not rob(t2).Corpse And rob(t2).FName = rob(t).FName Then  ' Must exist, and be of same species
  ...
  For ecocount = 1 To 15
      ...
      sPopulation = (IIf(intFindBestV2 < 100, 100, 200 - intFindBestV2)) / 100
        ...
        'save and kill the robot
        salvarob fit, MDIForm1.MainDir & "\evolution\baserob" & ecocount & "\Base.txt", True
        salvarob fit, MDIForm1.MainDir & "\evolution\mutaterob" & ecocount & "\Mutate.txt", True
        rob(fit).exist = False
